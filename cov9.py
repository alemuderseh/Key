...
# Merge the datasets on the 'countriesAndTerritories' column
merged_df = pd.merge(df, geo_data, how='left', left_on='countriesAndTerritories', right_on='country')

# Calculate the rate of COVID-19 cases per 100,000 population
merged_df['cases_per_100k'] = merged_df['cases'] * 100000 / merged_df['popData2019']
...
merged_df['cases_per_100k_pop'] = pd.cut(merged_df['cases_per_100k'], bins=bins, labels=labels, right=False)

# Load GeoJSON data for world boundaries
with open('countries.geojson', 'r') as f:
    world_geojson = json.load(f)

# Plot the choropleth map with categorized colors
fig = px.choropleth(
    merged_df,
    geojson=world_geojson,
    locations='countriesAndTerritories',
    featureidkey='properties.name',
    color='cases_per_100k_pop',
    color_discrete_map={
        '0-5': 'green',
        '5-10': 'yellow',
        '10-15': 'orange',
